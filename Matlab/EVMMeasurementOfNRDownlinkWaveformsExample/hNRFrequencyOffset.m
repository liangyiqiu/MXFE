...
%
%   [OUT,AVERAGEFO] =
%   hNRFrequencyOffset('FINEFO',CARRIER,BWPCFG,WAVEFORM,...
%                      REFGRID,IDEALGRID,CDMLENGTHS,SAMPLERATE,ACTIVESLOTS)
...
%                  length of frequency domain code division multiplexing
%                  (FD-CDM) and time domain code division multiplexing
%                  (TD-CDM) despreading to perform. A value of 1 for an
%                  element indicates no CDM and a value greater than 1
%                  indicates the length of the CDM
%   SAMPLERATE   - Sample rate of the waveform
%   ACTIVESLOTS  - A vector corresponding to the allocated slots present
%                  in the waveform WAVEFORM
%   OUT          - Output waveform with the estimated fine frequency
%                  offset correction
%   AVERAGEFO    - Output FO averaged across the allocated slots present
%                  in the waveform WAVEFORM
%
...
%
%   FOFFSET =
%   hNRFrequencyOffset('INTEGERFO',CARRIER,WAVEFORM,REFGRID,SAMPLERATE)
...
% Copyright 2022 The MathWorks, Inc.
    ...
    switch operationType
        ...
        case 'fineFO'
            ...
            rxGrid = nrOFDMDemodulate(carrier,waveform,'SampleRate',sampleRate);
            ...
            for slotIdx = 0:nSlots-1
                ...
                if ~isempty(find(currentSlot == activeSlots,1))
                    ...
                    % Correct the slot with the estimated fine FO
                    if slotwiseFineFOEnabled
                        rxSlot = nrOFDMModulate(carrier,rxGrid(:,symIdx,:),'SampleRate',sampleRate);
                        rxSlotFreqCorrected = frequencyOffsetCorrect(rxSlot,sampleRate,foffsetEstFine);
                    ...
                    if slotwiseFineFOEnabled
                        rxSlotFreqCorrected = nrOFDMModulate(carrier,rxGrid(:,symIdx,:),'SampleRate',sampleRate);
                    end
                end
        ...
        case 'integerFO'
            ...
            varargout{1} = frequencyIntegerCFO(carrier,waveform,refGrid,sampleRate);
...
function [integerCfoHz] = frequencyIntegerCFO(carrier,waveform,refGrid,sampleRate)
%   OUT = frequencyIntegerCFO(CARRIER,WAVEFORM,REFGRID,SAMPLERATE)
